...
const LoginScreen: React.FC<Props> = ({ onLangSelect, lang }) => {
  ...
  const T = {
    fr: { title: 'L\'application de rencontre pour les Burundais du monde entier', community: 'Rejoins la communauté Urukundo', google: 'Continuer avec Google', connecting: 'Connexion...', error: 'Erreur de connexion. Réessaie.', ios: 'iPhone détecté — tu seras redirigé vers Google', diaspora: 'Diaspora', meetings: 'Rencontres', chat: 'Chat' },
    en: { title: 'The dating app for Burundians around the world', community: 'Join the Urukundo community', google: 'Continue with Google', connecting: 'Connecting...', error: 'Connection error. Try again.', ios: 'iPhone detected — you will be redirected to Google', diaspora: 'Diaspora', meetings: 'Dating', chat: 'Chat' }
  };
  const t = T[lang];
  ...
  return (
    <div className="flex flex-col h-screen bg-white">
      ...
      <div className="p-8 space-y-4 bg-white">
        ...
        {isIOS && <p className="text-xs text-gray-400 text-center">📱 {t.ios}</p>}
      </div>
    </div>
  );
};
